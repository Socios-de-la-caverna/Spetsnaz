...
export class examenAprobacion{
    static async  aceptar(interaction:CommandInteraction) {
        ...
        const usuario_Member_HighestRole:any | string = (await usuario_Member)?.roles.highest.position;

        const usuario_Que_Ejecuto_El_Comando = interaction.guild?.members.fetch(interaction.user.id)
        ...
        if (usuario_Member_HighestRole > bot_Member?.roles.highest.position){
            return await interaction.reply('¡Ese usuario tiene un rol mas alto que el mio, no lo puedo aceptar!')
        }
    ...
    static async rechazar (interaction:CommandInteraction){
        ...
        if (usuario_Member_HighestRole > bot_Member?.roles.highest.position){
            return await interaction.reply('¡Ese usuario tiene un rol mas alto que el mio, no lo puedo expulsar!')
        }
        if (usuario_Que_Ejecuto_El_Comando == usuario_Member){
        ...
        await interaction.reply(`Usuario <@${(await usuario_Member)?.id}> expulsado exitosamente.`)
    }
}